// simulator-main 默认配置 Rename 边界的 BSD 封装。
//
// 参考结构体：
...
//   po = {ren2dec, ren2dis}
//
// 当前裁剪版不携带调试/统计旁带，Rename 边界只保留功能字段。
...
module ren_top #(
    ...
    ren_bsd_top #(
        .W_RenIn(W_RenIn),    // 实际值：2667 bit，main/default 的重命名输入总线宽度。
        .W_RenOut(W_RenOut)   // 实际值：1881 bit，main/default 的重命名输出总线宽度。
    ) u_ren_bsd_top (
        .clk(clk),
        .rst_n(rst_n),
        .pi(pi),
