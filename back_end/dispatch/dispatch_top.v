// simulator-main 默认配置 Dispatch 边界的 BSD 封装。
//
// 参考结构体：
...
//
// 该封装只负责把 simulator-main 的 Dispatch 输入/输出结构按字段顺序打包。
// 下方展开的 wire 是本地可读性视图，用于组员核对字段顺序，不改变 BSD 的 pi/po 边界。
...
module dispatch_top #(
    ...
    dispatch_bsd_top #(
        .W_DisIn(W_DisIn),    // 实际值：2386 bit，main/default 的 Dispatch 输入总线宽度。
        .W_DisOut(W_DisOut)   // 实际值：10577 bit，main/default 的 Dispatch 输出总线宽度。
    ) u_dispatch_bsd_top (
        .clk(clk),
        .rst_n(rst_n),
        .pi(pi),
